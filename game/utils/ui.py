import pygame
import os
import json
import zipfile
import math
import random
from pygame import SRCALPHA
from game.utils.util_functs import *
import __main__ as main

def is_tool_item(item_id):
    return item_id in main.item_data and "ToolType" in main.item_data[item_id]


def slot_stack_limit(item_id):
    if item_id == 0:
        return 256
    return main.item_data[item_id].get("StackSize", 256)


def draw_durability_bar(surface, x, y, slot_item_id, durability_used):
    if slot_item_id == 0 or slot_item_id not in main.item_data:
        return
    item_data = main.item_data[slot_item_id]
    max_dur = item_data.get("MaxDurability", 0)
    if max_dur <= 0:
        return

    durability_left = max(max_dur - durability_used, 0)
    ratio = durability_left / max_dur
    bar_w = 48
    bar_h = 6
    bg_rect = pygame.Rect(x, y + 42, bar_w, bar_h)
    fg_rect = pygame.Rect(x + 1, y + 43, max(int((bar_w - 2) * ratio), 1 if durability_left > 0 else 0), bar_h - 2)
    pygame.draw.rect(surface, (25, 25, 25), bg_rect)
    color = (220, 50, 40) if ratio < 0.33 else (230, 170, 40) if ratio < 0.66 else (70, 210, 80)
    pygame.draw.rect(surface, color, fg_rect)

def ui(events, surf, scale):
    surface = pygame.Surface((1920, 1080), SRCALPHA)

    for event in events:
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE and not main.show_inv:
                main.show_esc = not main.show_esc
            elif event.key == pygame.K_e and not main.show_esc:
                if main.show_inv:
                    main.container_open[0] = False
                main.show_inv = not main.show_inv
            elif event.key == pygame.K_F1:
                main.show_debug = not main.show_debug

    #Block selector
    mouse = pygame.mouse.get_pos()
    if not main.show_inv and not main.show_esc:
        grid_size = 16 * scale

        block_x = grid_size * ((mouse[0] - main.OX) // grid_size) + main.OX
        block_y = grid_size * ((mouse[1] - main.OY) // grid_size) + main.OY

        if point_distance((surf.get_width() / 2, surf.get_height() / 2), (mouse[0], mouse[1])) < main.reach:
            main.block_in_reach = True
            sel_col = (255,255,0)
        else:
            main.block_in_reach = False
            sel_col = (255, 0, 0)

        if main.break_progress > 0:
            break_sprite = pygame.transform.scale(pygame.image.load(resource_path(f"game/assets/blocks/destroy/destroy_stage_{clamp(round((10*main.break_progress)/100)-1, 0, 9)}.png")), (64, 64))
            surface.blit(break_sprite, (block_x, block_y))
        pygame.draw.rect(surface, sel_col, (block_x, block_y, (16 * scale), (16 * scale)), 3)
        surf.blit(surface, (0,0))

    #change cursor
    x = int(((mouse[0] - main.OX) % 4096) // 64)
    y = int(((mouse[1] - main.OY) % 4096) // 64)
    mouse_chunk = mouse_get_chunk()
    hovered_block_id = main.loaded_chunks[mouse_chunk][0][x][y]
    hovered_block_info = main.block_data.get(hovered_block_id, main.block_data[0])
    if hovered_block_info["Interactable"]:
        main.cur = main.cur_circle

    #ESC Screen
    if main.show_esc:
        pygame.draw.rect(surf, (68, 41, 31), (460, 190, 1000, 700))

        if button_exact(960, 250, 400, 50, main.img_button, (37, 124, 211, 100), "Resume", main.surface, events, "M", "T") == "left":
            main.show_esc = False
        if button_exact(960, 350, 400, 50, main.img_button, (37, 124, 211, 100), "Save", main.surface, events, "M", "T") == "left":
            save_world()
            main.show_esc = False
            main.img_save_timeout = 5
        if button_exact(960, 450, 400, 50, main.img_button, (37, 124, 211, 100), "Save & Quit", main.surface, events, "M", "T") == "left":
            save_world()
            main.show_esc = False
            main.img_save_timeout = 1

    #hotbar
    surf.blit(main.img_hotbar, (704, 1016))
    surf.blit(main.img_hotbar_sel, (704+(64*main.hotbar_slot), 1016))

    for i in range(8):
        if main.inventory[i][0] != 0:
            text = main.fnt_cons20.render(f"{main.inventory[i][1]}", True, (255, 255, 255))
            surf.blit(main.item_data[main.inventory[i][0]]["Texture"], (712 + 64 * i, 1024))
            surf.blit(text, (714 + 64 * i, 1055))
            draw_durability_bar(surf, 712 + 64 * i, 1024, main.inventory[i][0], main.inventory_durability[i])

    for event in events:
        if event.type == pygame.MOUSEWHEEL:
            main.hotbar_slot -= event.y
            if main.hotbar_slot < 0:
                main.hotbar_slot = 7
            elif main.hotbar_slot > 7:
                main.hotbar_slot = 0
            main.action_title = [main.fnt_cons20.render(main.item_data[main.inventory[main.hotbar_slot][0]]["Name"], True, (230, 230, 230, clamp(main.action_title[1], 0, 255))), 700]
    if main.inventory[main.hotbar_slot][0] < 1000:
        main.block_in_hand = main.inventory[main.hotbar_slot][0]
    else:
        main.block_in_hand = 0

    if main.action_title[1] >= 2:
        text = main.action_title[0].copy()
        text.set_alpha(main.action_title[1])
        surf.blit(text, (surf.get_width() / 2 - main.action_title[0].get_width() / 2, 950))
        main.action_title[1] -= 5

    #container ui's
    if main.container_open[0]:
        cont_x = 632
        cont_y = 150
        match main.container_open[1]:
            case 15:
                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x, cont_y, 656, 336), 0, 12)
                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x + 228, cont_y - 40, 200, 50), 0, 12)
                button(cont_x + 178, cont_y - 40, 300, 50, main.img_empty, (0, 0, 0, 0), "Workbench", main.surface, events, "L", "T")
                surf.blit(main.img_double_arrow, (cont_x+320, cont_y+120))
                recipe = []
                slot = 0
                for y in range(3):
                    for x in range(3):
                        btn = button_exact(cont_x+50+x*70, cont_y+50+y*70, 64, 64, main.img_slot, (255, 255, 255, 50), 0, main.surface, events, "L", "T")
                        if btn == "left":
                            if main.workbench_storage[slot][0] == main.inv_mouse[0]:
                                main.workbench_storage[slot][1] += main.inv_mouse[1]
                                main.inv_mouse = [0, 0]
                                main.inv_mouse_durability = 0
                            else:
                                temp = main.inv_mouse
                                main.inv_mouse = main.workbench_storage[slot]
                                main.workbench_storage[slot] = temp
                                main.inv_mouse_durability = 0
                        elif btn == "right":
                            if main.workbench_storage[slot][0] == 0 or main.workbench_storage[slot][0] == main.inv_mouse[0]:
                                main.workbench_storage[slot] = [main.inv_mouse[0], main.workbench_storage[slot][1]+1]
                                main.inv_mouse[1] -= 1
                                if main.inv_mouse[1] <= 0:
                                    main.inv_mouse = [0, 0]
                                    main.inv_mouse_durability = 0
                            elif main.inv_mouse == [0, 0] and main.workbench_storage[slot] != [0, 0]:
                                main.inv_mouse = [main.workbench_storage[slot][0], math.ceil(main.workbench_storage[slot][1] /2)]
                                main.workbench_storage[slot][1] = math.floor(main.workbench_storage[slot][1] / 2)
                                main.inv_mouse_durability = 0

                        if main.workbench_storage[slot][0] != 0:
                            text = main.fnt_cons20.render(f"{main.workbench_storage[slot][1]}", True, (255, 255, 255))
                            surf.blit(main.item_data[main.workbench_storage[slot][0]]["Texture"], (cont_x + x*70+58, cont_y + y*70+58))
                            surf.blit(text, (cont_x + x*70+60, cont_y + y*70+89))

                        recipe.append(main.workbench_storage[slot][0])

                        slot += 1
                main.workbench_storage[9] = [0, 0]
                for rec in main.recipe_data:
                    if rec[0] == recipe:
                        main.workbench_storage[9] = [rec[1],rec[2]]
                btn = button_exact(cont_x+444, cont_y+120, 64, 64, main.img_slot, (255, 255, 255, 50), 0, main.surface, events, "L", "T")
                if btn == "left" and main.workbench_storage[9][0] != 0:
                    result_item = main.workbench_storage[9][0]
                    result_amount = main.workbench_storage[9][1]
                    if main.inv_mouse[0] == 0:
                        main.inv_mouse = [result_item, result_amount]
                        main.inv_mouse_durability = 0
                    elif main.inv_mouse[0] == result_item:
                        max_stack = slot_stack_limit(result_item)
                        if main.inv_mouse[1] + result_amount <= max_stack:
                            main.inv_mouse = [result_item, main.inv_mouse[1] + result_amount]
                        else:
                            result_item = 0
                    else:
                        result_item = 0

                    if result_item != 0:
                        for i in range(9):
                            main.workbench_storage[i][1] -= 1
                            if main.workbench_storage[i][1] <= 0:
                                main.workbench_storage[i] = [0, 0]
                if main.workbench_storage[9][0] != 0:
                    text = main.fnt_cons20.render(f"{main.workbench_storage[9][1]}", True, (255, 255, 255))
                    surf.blit(main.item_data[main.workbench_storage[9 ][0]]["Texture"], (cont_x + 452, cont_y + 128))
                    surf.blit(text, (cont_x + 454, cont_y + 159))
            case 16:
                for chunk in main.container_savedata["Chunks"]:
                    if chunk["Coordinates"][0] == main.container_current[5][0] and chunk["Coordinates"][1] == main.container_current[5][1]:
                        for block in chunk["Blocks"]:
                            if block ["Coordinates"][0] == main.container_current[5][2] and block ["Coordinates"][0] == main.container_current[5][2]:
                                main.container_current = block["Data"]

                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x, cont_y, 656, 336), 0, 12)
                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x + 258, cont_y - 40, 140, 50), 0, 12)
                button(cont_x + 178, cont_y - 40, 300, 50, main.img_empty, (0, 0, 0, 0), "Furnace", main.surface, events, "L", "T")
                surf.blit(main.img_double_arrow, (cont_x + 296, cont_y + 136))
                if main.container_current[0][0] != 0:
                    width = math.floor(main.container_current[4] * 64) / 100
                    pygame.draw.rect(main.surface, (64, 64, 64), (cont_x + 296 + width, cont_y + 136, 65-width, 64))
                temp = main.img_fur_flame.copy()
                temp.set_alpha(main.furnace_timings[0])
                surf.blit(temp, (cont_x + 196, cont_y + 136))
                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x + 196, cont_y + 136, 64, math.floor((main.container_current[3] * 64) / 1000)))
                if main.daylight_time % 20 == 0:
                    main.furnace_timings[0] = random.randint(200, 255)

                if button_exact(cont_x + 228, cont_y + 98, 64, 64, main.img_slot, (255, 255, 255, 50), 0, main.surface, events, "M", "M") == "left":
                    if main.container_current[0][0] == main.inv_mouse[0]:
                        main.container_current[0][1] += main.inv_mouse[1]
                        main.inv_mouse = [0, 0]
                        main.inv_mouse_durability = 0
                    elif main.container_current[0][0] == 0:
                        main.container_current[0] = main.inv_mouse
                        main.inv_mouse = [0, 0]
                        main.inv_mouse_durability = 0
                    elif main.inv_mouse[0] == 0:
                        main.inv_mouse = main.container_current[0]
                        main.container_current[0] = [0, 0]
                        main.inv_mouse_durability = 0
                if button_exact(cont_x + 228, cont_y + 248, 64, 64, main.img_slot, (255, 255, 255, 50), 0, main.surface, events, "M", "M") == "left":
                    if main.container_current[1][0] == main.inv_mouse[0]:
                        main.container_current[1][1] += main.inv_mouse[1]
                        main.inv_mouse = [0, 0]
                        main.inv_mouse_durability = 0
                    elif main.container_current[1][0] == 0 and main.inv_mouse[0] in [1005, 34, 22, 28, 17, 15, 10, 9]:
                        main.container_current[1] = main.inv_mouse
                        main.inv_mouse = [0, 0]
                        main.inv_mouse_durability = 0
                    elif main.inv_mouse[0] == 0:
                        main.inv_mouse = main.container_current[1]
                        main.container_current[1] = [0, 0]
                        main.inv_mouse_durability = 0
                if button_exact(cont_x + 428, cont_y + 168, 64, 64, main.img_slot, (255, 255, 255, 50), 0, main.surface, events, "M", "M") == "left":
                    if main.inv_mouse[0] in [0, main.container_current[2][0]]:
                        main.inv_mouse[0] = main.container_current[2][0]
                        main.inv_mouse[1] += main.container_current[2][1]
                        main.container_current[2] = [0, 0]
                        main.inv_mouse_durability = 0

                if main.container_current[0][0] != 0:
                    surf.blit(main.item_data[main.container_current[0][0]]["Texture"], (cont_x + 204, cont_y + 74))
                    text = main.fnt_cons20.render(f"{main.container_current[0][1]}", True, (255, 255, 255))
                    surf.blit(text, (cont_x + 206, cont_y + 105))
                if main.container_current[1][0] != 0:
                    main.container_current[6] = main.container_current[1][0]
                    surf.blit(main.item_data[main.container_current[1][0]]["Texture"], (cont_x + 204, cont_y + 224))
                    text = main.fnt_cons20.render(f"{main.container_current[1][1]}", True, (255, 255, 255))
                    surf.blit(text, (cont_x + 206, cont_y + 255))
                if main.container_current[2][0] != 0:
                    surf.blit(main.item_data[main.container_current[2][0]]["Texture"], (cont_x + 404, cont_y + 144))
                    text = main.fnt_cons20.render(f"{main.container_current[2][1]}", True, (255, 255, 255))
                    surf.blit(text, (cont_x + 406, cont_y + 175))

                if main.container_current[0][1] <= 0:
                    main.container_current[0][0] = 0
                if main.container_current[1][1] <= 0:
                    main.container_current[1][0] = 0

                for chunk in main.container_savedata["Chunks"]:
                    if chunk["Coordinates"][0] == main.container_current[5][0] and chunk["Coordinates"][1] == main.container_current[5][1]:
                        for block in chunk["Blocks"]:
                            if block["Coordinates"][0] == main.container_current[5][2] and block["Coordinates"][0] == main.container_current[5][2]:
                                block["Data"] = main.container_current

            case 17:
                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x, cont_y, 656, 336), 0, 12)
                pygame.draw.rect(main.surface, (64, 64, 64), (cont_x + 268, cont_y - 40, 120, 50), 0, 12)
                button(cont_x + 178, cont_y - 40, 300, 50, main.img_empty, (0, 0, 0, 0), "Chest", main.surface, events, "L", "T")

                slot = 0
                for y in range(4):
                    for x in range(8):
                        btn = button_exact(cont_x + (x * 80) + 16, cont_y + (y * 80) + 16, 64, 64, main.img_slot, (255, 255, 255, 50), 0, main.surface, events, "L", "T")
                        if btn == "left":
                            if main.inv_mouse[0] == main.container_current[slot][0]:
                                if main.container_current[slot][1] + main.inv_mouse[1] <= 256:
                                    main.container_current[slot][1] += main.inv_mouse[1]
                                    main.inv_mouse = [0, 0]
                                    main.inv_mouse_durability = 0
                                else:
                                    main.inv_mouse[1] -= 256 - main.container_current[slot][1]
                                    main.container_current[slot][1] = 256
                            else:
                                temp = main.inv_mouse
                                main.inv_mouse = main.container_current[slot]
                                main.container_current[slot] = temp
                                main.inv_mouse_durability = 0
                        elif btn == "right":
                            if main.inv_mouse == [0, 0] and main.container_current[slot] != [0, 0]:
                                main.inv_mouse = [main.container_current[slot][0], math.ceil(main.container_current[slot][1] / 2)]
                                main.container_current[slot][1] = math.floor(main.container_current[slot][1] / 2)
                                main.inv_mouse_durability = 0
                            else:
                                if main.container_current[slot] == [0, 0] or main.container_current[slot][0] == main.inv_mouse[0]:
                                    if main.container_current[slot][1] + 1 <= 256:
                                        main.container_current[slot][0] = main.inv_mouse[0]
                                        main.container_current[slot][1] += 1
                                        main.inv_mouse[1] -= 1

                        if main.container_current[slot][0] != 0:
                            text = main.fnt_cons20.render(f"{main.container_current[slot][1]}", True, (255, 255, 255))
                            surf.blit(main.item_data[main.container_current[slot][0]]["Texture"],
                                      (cont_x + 80 * x + 24, cont_y + 80 * y + 24))
                            surf.blit(text, (cont_x + 80 * x + 26, cont_y + 80 * y + 55))
                        slot += 1

    #Inventory
    if main.show_inv:
        inv_x = 632
        inv_y = 600

        for i in range(8):
            if button(704 + i * 64, 1016, 64, 64, main.img_empty, (255, 255, 255, 50), 0, main.surface, events, "L", "T"):
                temp = main.inv_mouse
                main.inv_mouse = main.inventory[i]
                main.inventory[i] = temp
                temp_d = main.inv_mouse_durability
                main.inv_mouse_durability = main.inventory_durability[i]
                main.inventory_durability[i] = temp_d

        slot = 8
        pygame.draw.rect(main.surface, (64, 64, 64), (inv_x, inv_y, 656, 336), 0, 12)
        pygame.draw.rect(main.surface, (64, 64, 64), (inv_x + 238, inv_y - 40, 180, 50), 0, 12)
        button(inv_x + 178, inv_y - 40, 300, 50, main.img_empty, (0, 0, 0, 0), "Inventory", main.surface, events, "L", "T")
        for y in range(4):
            for x in range(8):
                btn = button_exact(inv_x + (x * 80) + 16, inv_y + (y * 80) + 16, 64, 64, main.img_slot,
                                   (255, 255, 255, 50), 0, main.surface, events, "L", "T")
                if btn == "left":
                    if main.inv_mouse[0] == main.inventory[slot][0]:
                        if main.inventory[slot][1] + main.inv_mouse[1] <= slot_stack_limit(main.inventory[slot][0]):
                            if not is_tool_item(main.inv_mouse[0]):
                                main.inventory[slot][1] += main.inv_mouse[1]
                                main.inv_mouse = [0, 0]
                                main.inv_mouse_durability = 0
                        else:
                            if not is_tool_item(main.inv_mouse[0]):
                                main.inv_mouse[1] -= slot_stack_limit(main.inventory[slot][0]) - main.inventory[slot][1]
                                main.inventory[slot][1] = slot_stack_limit(main.inventory[slot][0])
                    else:
                        temp = main.inv_mouse
                        main.inv_mouse = main.inventory[slot]
                        main.inventory[slot] = temp
                        temp_d = main.inv_mouse_durability
                        main.inv_mouse_durability = main.inventory_durability[slot]
                        main.inventory_durability[slot] = temp_d
                elif btn == "right":
                    if main.inv_mouse == [0, 0] and main.inventory[slot] != [0, 0]:
                        main.inv_mouse = [main.inventory[slot][0], math.ceil(main.inventory[slot][1] / 2)]
                        main.inventory[slot][1] = math.floor(main.inventory[slot][1] / 2)
                        if main.inv_mouse[1] > 0:
                            main.inv_mouse_durability = main.inventory_durability[slot]
                        if main.inventory[slot][1] <= 0:
                            main.inventory_durability[slot] = 0
                    else:
                        if main.inventory[slot] == [0, 0] or main.inventory[slot][0] == main.inv_mouse[0]:
                            if main.inventory[slot][1] + 1 <= slot_stack_limit(main.inv_mouse[0]):
                                if not (is_tool_item(main.inv_mouse[0]) and main.inventory[slot][1] > 0):
                                    main.inventory[slot][0] = main.inv_mouse[0]
                                    main.inventory[slot][1] += 1
                                    main.inv_mouse[1] -= 1
                                    if main.inventory[slot][1] == 1:
                                        main.inventory_durability[slot] = main.inv_mouse_durability

                if main.inventory[slot][0] != 0:
                    text = main.fnt_cons20.render(f"{main.inventory[slot][1]}", True, (255, 255, 255))
                    surf.blit(main.item_data[main.inventory[slot][0]]["Texture"],
                              (inv_x + 80 * x + 24, inv_y + 80 * y + 24))
                    surf.blit(text, (inv_x + 80 * x + 26, inv_y + 80 * y + 55))
                    draw_durability_bar(surf, inv_x + 80 * x + 24, inv_y + 80 * y + 24, main.inventory[slot][0], main.inventory_durability[slot])
                slot += 1

        if main.gamemode == 1:
            cr_inv_x = 1380
            cr_inv_y = 40
            rect_surf = pygame.Surface((1000, 1000), pygame.SRCALPHA)
            pygame.draw.rect(rect_surf, (0, 0, 0, 180), (0, 0, 500, 1000), 0, 12)
            surf.blit(rect_surf, (cr_inv_x, cr_inv_y))

            btn_surf = pygame.Surface((100, 40), pygame.SRCALPHA)
            pygame.draw.rect(btn_surf, (64, 64, 64), (0, 0, 100, 40), 0, 8)

            if button(cr_inv_x +20, cr_inv_y +20, 100, 40, btn_surf, (255, 255, 255, 100), "Blocks", surf, events, "L", "T"):
                main.creative_menu_category = 0
            if button(cr_inv_x +130, cr_inv_y +20, 100, 40, btn_surf, (255, 255, 255, 100), "Items", surf, events, "L", "T"):
                main.creative_menu_category = 1

            if main.creative_menu_category == 0:
                item_id = 0
            else:
                item_id = 1000
            for y in range(14):
                for x in range(6):
                    item_id += 1
                    if item_id in main.item_data:
                        btn = button_exact(cr_inv_x + 20 + 78*x, cr_inv_y + 80 + 78*y, 64, 64, main.item_data[item_id]["Texture"], (255, 255, 255, 100), 0, main.surface, events, "L", "T")

                        if btn == "left":
                            if main.inv_mouse[0] == item_id:
                                if main.inv_mouse[1] < slot_stack_limit(item_id):
                                    main.inv_mouse[1] += 1
                            else:
                                main.inv_mouse = [item_id, 1]
                                main.inv_mouse_durability = 0
                        elif btn == "right":
                            main.inv_mouse = [item_id, 256]
                            main.inv_mouse_durability = 0

            if button(cr_inv_x +420, cr_inv_y +920, 64, 64, main.trashbin, (255, 255, 255, 100), 0, surf, events, "L", "T"):
                main.inv_mouse = [0, 0]
                main.inv_mouse_durability = 0

        elif main.gamemode == 0:
            pass

        if main.inv_mouse[1] <= 0:
            main.inv_mouse[0] = 0
            main.inv_mouse_durability = 0

        if main.inv_mouse[0] != 0:
            text = main.fnt_cons20.render(f"{main.inv_mouse[1]}", True, (255, 255, 255))
            surf.blit(main.item_data[main.inv_mouse[0]]["Texture"], (mouse[0]-24, mouse[1]-24))
            surf.blit(text, (mouse[0]-22, mouse[1]+7))
            draw_durability_bar(surf, mouse[0]-24, mouse[1]-24, main.inv_mouse[0], main.inv_mouse_durability)

    #ui elements added by mods
    if main.mods_active:
        for mod in main.loaded_mods:
            with zipfile.ZipFile(f"{main.MODPATH}/{mod}", 'r') as zip_ref:
                if "scripts/ui.py" in zip_ref.namelist():
                    with zip_ref.open("scripts/ui.py") as file:
                        exec(file.read())


    #Debug Infos
    if main.show_debug:
        pygame.draw.rect(main.surface, (250, 1, 209), (main.OX, main.OY, 4096, 4096), 3)
        mx = int(((mouse[0] - main.OX) % 4096) // 64)
        my = int(((mouse[1] - main.OY) % 4096) // 64)
        total_entities = len(main.pig_entities) + len(main.cow_entities) + len(main.sheep_entities) + len(main.item_entities)
        main_chunk_entities = 0
        for entity in main.pig_entities + main.cow_entities + main.sheep_entities + main.item_entities:
            entity_chunk_x = math.floor(entity.x / 4096)
            entity_chunk_y = math.floor(entity.y / 4096)
            if entity_chunk_x == 0 and entity_chunk_y == 0:
                main_chunk_entities += 1
        debug_txt = (f"FPS: {main.clock.get_fps():.2f}\n"
                     f"OriginX: {main.OX}, OriginX: {main.OY}\n"
                     f"MouseX: {mouse[0]}\nMoseY: {mouse[1]}\n"
                     f"MouseChunk: {mouse_get_chunk()}\n"
                     f"BlockHover ID: {main.loaded_chunks[mouse_get_chunk()][0][mx][my]},  X: {mx},  Y: {my}\n"
                     f"MainChunk: {main.loaded_chunks[4][1]}\n"
                     f"Entities Total: {total_entities}\n"
                     f"Entities In MainChunk: {main_chunk_entities}\n"
                     f"Rendering Chunks: {len(main.chunk_render_queue) > 0} "
                     f"({', '.join(str(chunk) for chunk, *_ in main.chunk_render_queue)})\n"
                     f"{f"Rendering Chunk: {main.chunk_render_queue[0][0]} - {((4096 - len(main.chunk_render_queue[0][1])) * 100) / 4096:.0f}%\n" if main.chunk_render_queue else ""}"
                     f"Buffered Chunks: {len(main.chunk_buffer)}\n"
                     f"Daytime: {main.daylight_time}\n"
                     )
        text_render_multiline(10, 10, main.main_font, debug_txt, True, (255, 255, 255), surf, "L", "T")

